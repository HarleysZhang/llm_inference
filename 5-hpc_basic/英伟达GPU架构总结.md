...
即 L1 数据缓冲和共享内存合并，共用一个硬件内存块，首次引入于 Volta V100 芯片，NVIDIA 的**结合 L1 数据缓存和共享内存**子系统架构显著提高了性能，同时简化了编程，减少了达到或接近应用性能峰值所需的调优工作。将数据缓存和共享内存功能合并到一个内存块中，为两种类型的内存访问提供了最佳的整体性能。A100 的 L1 数据缓存和共享内存的总容量为 192 KB/SM，而 V100 为 128 KB/SM。
